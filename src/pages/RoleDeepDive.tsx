...
const RoleDeepDive = () => {
    ...
    (async () => {
      try {
        ...
        const { data: links } = await supabase
          ...
          .select("job_id, jobs!inner(id, title, companies(name))")
  ...
  const [simLevel, setSimLevel] = useState<1 | 2>(levelParam === "2" ? 2 : 1);
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-5xl w-[95vw] h-[85vh] p-0 gap-0 overflow-hidden rounded-2xl border-border/60 bg-background">
        ...
        <SimulatorModal
          open={!!simTask}
          onClose={() => { setSimTask(null); setCurrentIntel(null); setSimLevel(1); setSimFuturePrediction(undefined); fetchCompletions(); }}
