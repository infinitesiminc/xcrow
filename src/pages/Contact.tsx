...
type Segment = "school" | "enterprise" | "investor";

const SEGMENTS: { key: Segment; label: string; icon: typeof GraduationCap; desc: string; orgLabel: string; orgPlaceholder: string; emailPlaceholder: string; messagePlaceholder: string }[] = [
  { key: "school", label: "Universities", icon: GraduationCap, desc: "Curriculum integration, student skill tracking & employability reporting", orgLabel: "University Name", orgPlaceholder: "e.g. UCLA, NYU, University of Michigan", emailPlaceholder: "you@university.edu", messagePlaceholder: "Tell me about your career services goals, student cohort size, or specific challenges…" },
  { key: "enterprise", label: "Employers", icon: Building2, desc: "Verified talent pipelines, ATS integrations & skill-based hiring", orgLabel: "Company Name", orgPlaceholder: "e.g. Deloitte, Google, McKinsey", emailPlaceholder: "you@company.com", messagePlaceholder: "Tell me about your hiring challenges, team size, or how you currently assess AI-readiness…" },
  { key: "investor", label: "Investors", icon: Briefcase, desc: "Seed round, partnership opportunities & strategic alignment", orgLabel: "Fund / Firm", orgPlaceholder: "e.g. a16z, Sequoia, Angel", emailPlaceholder: "you@fund.com", messagePlaceholder: "Tell me about your investment thesis, check size, or what caught your interest…" },
];

const Contact = () => {
  const { toast } = useToast();
  const [segment, setSegment] = useState<Segment>("school");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke("send-contact-email", {
        body: { name, email, subject: `${segment}-inquiry`, company: org, teamSize: "", message: message || `${seg.label} inquiry from ${org}`, formType: "org", phone },
      });
      ...
      <div className="mx-auto max-w-5xl px-4 py-16 sm:py-24">
        ...
        <motion.div {...fade()} className="max-w-2xl mx-auto text-center mb-12">
          ...
          <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
            I'm <span className="text-foreground font-semibold">Jackson</span>, founder of Xcrow. Whether you're a university preparing students for AI, an employer hiring for skills that don't exist on resumes yet, or an investor backing the next wave of workforce infrastructure — I'd love to hear from you.
          </p>
        </motion.div>

        {/* ═══ SEGMENT PICKER ═══ */}
        <motion.div {...fade(0.1)} className="flex flex-wrap justify-center gap-3 mb-10">
          {SEGMENTS.map((s) => {
            const Icon = s.icon;
            ...
            return (
              <button
                key={s.key}
                onClick={() => { setSegment(s.key); setSent(false); }}
                className={`flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-medium transition-all border ${active ? "border-primary bg-primary/10 text-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/30 hover:text-foreground"}`}
              >
        ...
        {/* ═══ SEGMENT VALUE PROP ═══ */}
        <motion.p {...fade(0.12)} className="text-center text-sm text-muted-foreground mb-10 max-w-lg mx-auto">
          {seg.desc}
        </motion.p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      <label className="text-sm font-medium mb-1.5 block">
                        <seg.icon className="inline h-3.5 w-3.5 mr-1.5 -mt-0.5 text-primary" />
                        {seg.orgLabel} *
                      </label>
                      <input type="text" value={org} onChange={e => setOrg(e.target.value)}
                        required maxLength={150} className={inputClass} placeholder={seg.orgPlaceholder} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                          required maxLength={255} className={inputClass} placeholder={seg.emailPlaceholder} />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea value={message} onChange={e => setMessage(e.target.value)}
                        ...
                        placeholder={seg.messagePlaceholder} />
                    </div>
