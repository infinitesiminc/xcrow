/**
 * Play — Universal landing page for anyone who wants to build career skills
 * through the RPG game format. Showcases the progression loop visually.
 */
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                ...
                >
                  ...
                  {!s.img && <div className="h-40 flex items-center justify-center" style={{ background: `linear-gradient(135deg, hsl(${s.color} / 0.1), transparent)` }}>
                    <Crown className="h-16 w-16 opacity-20" style={{ color: `hsl(${s.color})` }} />
                  </div>}
                  <div className="p-6">
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {CASTLE_STAGES.map((c, i) => (
                ...
                >
                  ...
                  {i < CASTLE_STAGES.length - 1 && (
                    <ArrowRight className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/30" />
                  )}
                </motion.div>
              ))}
            </div>
            ...
            <div className="hidden md:flex justify-center mt-2">
              <div className="flex items-center gap-1 text-muted-foreground/30">
                {["🏚️", "→", "🏕️", "→", "🏰", "→", "⚔️"].map((s, i) => (
                  <span key={i} className={`text-lg ${s === "→" ? "text-sm" : ""}`}>{s}</span>
                ))}
              </div>
            </div>
          </div>
              ...
              {RANK_LADDER.map((r, i) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ height: 3, background: r.color, opacity: 0.5 }}
                  />
                </motion.div>
              ))}
            </div>
