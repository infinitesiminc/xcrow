/**
 * OrgStackMapper — Enterprise org-wide tool mapping extracted from OrgStack page.
 * Displays company departments and their mapped AI tools from job data.
 */
import { useState, useEffect, useMemo } from "react";
import { supabase } from "@/integrations/supabase/client";
import { GTC_TOOLS, type GTCTool, CATEGORY_CONFIG } from "@/data/gtc-tools-registry";
...
export default function OrgStackMapper() {
  ...
  async function loadCompany(name: string) {
    ...
    try {
      ...
      for (let i = 0; i < jobIds.length; i += batchSize) {
        ...
        const { data: skills } = await supabase.from("job_skills").select("job_id, name").in("job_id", batch);
      ...
      for (let i = 0; i < jobIds.length; i += batchSize) {
        const batch = jobIds.slice(i, i + batchSize);
        const { data: tasks } = await supabase.from("job_task_clusters").select("job_id, skill_names, cluster_name").in("job_id", batch);
      ...
      for (const t of allTaskSkills) {
        ...
        if (t.skill_names) { for (const sn of t.skill_names) jobSkillsMap.get(t.job_id)!.add(sn); }
      ...
      for (const [dept, { jobs: deptJobs }] of deptMap) {
        ...
        for (const job of deptJobs) {
          ...
          for (const [toolName, matchedSkills] of toolMap) {
            ...
            if (!deptTools.has(toolName)) deptTools.set(toolName, { score: 0, jobTitles: new Set(), skills: new Set() });
        ...
        for (const [toolName, data] of deptTools) {
          if (!orgToolMap.has(toolName)) orgToolMap.set(toolName, { score: 0, deptCount: 0, departments: new Set() });
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "hsl(var(--primary))" }}>
            Enterprise Stack Mapper
          </span>
        </div>
        <h2 className="text-xl font-black mb-1" style={{ color: "hsl(var(--foreground))" }}>
          Map Your Organization's AI Stack
        </h2>
      ...
      {orgData && !loading && conflicts.ownTools.length > 0 && (
        <div className="mb-6 rounded-xl p-3 flex items-start gap-3"
          ...
          <div>
            <p className="text-xs font-bold mb-1" style={{ color: "hsl(var(--destructive))" }}>Competitive Conflict Detection</p>
            <p className="text-[11px] leading-relaxed" style={{ color: "hsl(var(--foreground) / 0.7)" }}>
              {orgData.companyName} builds <span className="font-semibold">{conflicts.ownTools.join(", ")}</span>.
              {conflictCount > 0 && <> Found <span className="font-bold" style={{ color: "hsl(var(--destructive))" }}>{conflictCount} competitor tools</span> that should be replaced with internal alternatives.</>}
          ...
          <div className="lg:col-span-3">
            <div className="rounded-2xl p-4" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border) / 0.15)" }}>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-bold" style={{ color: "hsl(var(--foreground))" }}>{orgData.companyName}</h2>
              ...
              >
                <Layers className="h-3.5 w-3.5" /> Org-Wide Overview
              </button>
                    ...
                    >
                      ...
                      <p className="text-[10px]" style={{ color: "hsl(var(--muted-foreground))" }}>
                        {conflictWith
                          ...
                          : <>{Array.from(t.departments).slice(0, 3).join(", ")}{t.departments.size > 3 && ` +${t.departments.size - 3}`}</>
                        }
                      </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                ...
                {deptTools.length === 0 && (
                  <p className="col-span-full text-center py-12 text-sm" style={{ color: "hsl(var(--muted-foreground))" }}>
                    No tool data for this department yet.
                  </p>
                )}
              </div>
            )}
          ...
          <div className="lg:col-span-3">
            {selectedTool ? (
              <div className="rounded-2xl p-4 sticky top-24" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border) / 0.15)" }}>
                ...
                <p className="text-xs mb-3 leading-relaxed" style={{ color: "hsl(var(--foreground) / 0.7)" }}>{selectedTool.description}</p>
                ...
                <div className="mb-4">
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: "hsl(var(--muted-foreground))" }}>Department Coverage</p>
                  <div className="flex flex-wrap gap-1">
                    {Array.from(orgData.allTools.get(selectedTool.name)?.departments || []).map(d => (
                      <Badge key={d} variant="outline" className="text-[9px] cursor-pointer" onClick={() => setSelectedDept(d)}>{d}</Badge>
                    ))}
                  </div>
                </div>
                ...
                {selectedTool.products && selectedTool.products.length > 0 && (
                  <div className="mb-4">
                    <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: "hsl(var(--muted-foreground))" }}>Product Suite</p>
              ...
              <div className="rounded-2xl p-4 sticky top-24" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border) / 0.15)" }}>
                ...
                <div className="space-y-3">
                  {(() => {
                    ...
                    for (const [name] of orgData.allTools) {
                      ...
                      if (t) catCounts.set(t.category, (catCounts.get(t.category) || 0) + 1);
                    }
                      ...
                      .map(([cat, count]) => (
                        <div key={cat}>
                          <div className="flex justify-between text-[11px] mb-1">
                            <span style={{ color: "hsl(var(--foreground) / 0.7)" }}>{cat.replace(/-/g, " ")}</span>
                          ...
                          <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "hsl(var(--muted) / 0.15)" }}>
                            <div className="h-full rounded-full" style={{ width: `${(count / orgData.allTools.size) * 100}%`, background: "hsl(var(--primary) / 0.6)" }} />
                          </div>
                        </div>
                      ));
                  })()}
                ...
                <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: "hsl(var(--muted-foreground))" }}>Cross-Department (3+ depts)</p>
                <div className="flex flex-wrap gap-1">
                  {topTools
                    .filter(t => t.deptCount >= 3)
                    .slice(0, 12)
                    .map(t => (
                      <Badge key={t.name} variant="secondary" className="text-[9px] cursor-pointer hover:opacity-80" onClick={() => setSelectedTool(t.tool)}>
                        {t.tool.icon} {t.name}
                      </Badge>
                    ))}
                </div>
